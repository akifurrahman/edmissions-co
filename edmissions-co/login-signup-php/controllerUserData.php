...
//require "../mailsenders/emailtest2.php";
//require "registrationEmail.php";
...
if(isset($_POST['signup'])){
    ...
    $errors['email'] = "This email adress isn't valid!";
    //exit(0);
    }
    ...
    if(strlen($password)<8){
        $errors['password'] = "Password length too small! Please enter a minimum 8 letter password";
    }
    ...
    if(count($errors) === 0){
        ...
        mysqli_stmt_bind_param($stmt, "ssssis", $name, $email, $encpass, $options, $code, $status);
...
$errors['otp-error'] = "Failed while sending code!";
    ...
    if(isset($_POST['check'])){
        ...
        //if($check_code==$_POST['otp']){ 
            ...
            //mysqli_stmt_bind_param($stmt, "is", $code,$status);
            //mysqli_stmt_execute($stmt);
            //$update_res = mysqli_stmt_get_result($stmt);
    ...
    if(isset($_POST['login'])){
        ...
        if(mysqli_num_rows($res) > 0){
            ...
            if(password_verify($password, $fetch_pass)){
                ...
                $errors['email'] = "Incorrect E-Mail or Password!";
            }
        }else{
            $errors['email'] = "Looks like you're not a member yet! Click on the Sign Up link to join.";
        }
    }
    ...
    if(isset($_POST['change-password'])){
        ...
        $errors['password'] = "Password length too small! Please enter a minimum 8 letter password";
            ...
            //$update_pass = "UPDATE usertable SET code = ?, password = ?, status= ? WHERE email = ?";

            //$stmt = mysqli_prepare($con, $update_pass);
            //mysqli_stmt_bind_param($stmt, "isss", $code,$encpass,$status,$email);
            //mysqli_stmt_execute($stmt);
            //$run_query = mysqli_stmt_get_result($stmt);
            ...
            if($run_query){
                $info = "Your password has been changed. Please proceed with logging in with your new password.";
